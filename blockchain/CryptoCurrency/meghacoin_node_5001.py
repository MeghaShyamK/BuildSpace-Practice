# here wee need requests library
...
def mine_block():
    ...
    blockchain.add_transaction(sender = node_address, receiver='Ashish', amount=1)
...
def is_valid():
    ...
    if is_valid:
        response= {'message':  'All goos Blockchain is valid'}
    else:
        response={'message': 'oops INVALID'}
    return jsonify(response), 200

@app.route('/add_transaction', methods=['POST'])
def add_transaction():
    json=request.get_json()
    transaxtion_keys=['sender','receiver','amount']
    if not all (key in json for key in transaxtion_keys):
...
def connect_node():
    ...
    response={'messages': 'ALl the nodes are now connected',
...
def replace_chain():
    ...
    if is_chain_replaced:
        response= {'message':  'Nodes had different chains so replaced by the longest chain',
